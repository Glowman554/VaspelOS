...
int create_window(int window_x, int window_y, int window_size_x, int window_size_y, char* name){
    for(int x = 0; x < window_size_x + 2; x++){
        for(int y = 0; y < 15; y++){
            setpixel(window_x + x, window_y - y, VGA_YELLOW);
        }
        setpixel(window_x + x, window_y + window_size_y + 1, VGA_YELLOW);
	}

    for(int y = 0; y < window_size_y + 2; y++){
        setpixel(window_x, window_y + y, VGA_YELLOW);
        setpixel(window_x + window_size_x + 1, window_y + y, VGA_YELLOW);
    }
...
void _start(void){
    init_vga();
    set_vga_color(VGA_YELLOW, VGA_DARKGRAY);
    clrscr();

    for(int x = 0; x < 320; x++){
        for(int y = 0; y < 15; y++){
            setpixel(x, y, VGA_YELLOW);
        }
	}

    set_vga_color(VGA_DARKGRAY, VGA_YELLOW);
    kprintf("VaspelOS %d from %s", kversion(), kvendor());
